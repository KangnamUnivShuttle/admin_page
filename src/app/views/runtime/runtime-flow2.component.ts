...
export class RuntimeFlow2Component implements OnInit, AfterViewInit {
  ...
  initData() {
    Promise.all([
      this.httpService
        .reqGet("runtimeBlock", { blockID: this.blockID })
        .toPromise(),
      this.httpService.reqGet("runtime", { blockID: this.blockID }).toPromise(),
      this.httpService
        .reqGet("runtimeLink", { blockID: this.blockID })
        .toPromise(),
    ]).then((res) => {
      console.log("res", res);
      ...
      runtime.data
        ...
        .forEach((run: BlockRuntimeModel, idx) => {
          ...
          run.image = {
            ...
            name: run._ChatImage_name,
            orderNum: run._ChatImage_order_num,
      ...
      this.childComponents.push({
        type: "link",
        ref: this.guid(),
        x: start.data[1].linkX,
        y: start.data[1].linkY,
        w: 0,
        h: 0,
        orderNum: 99999,
        data: link.data.slice(1, link.data.length),
      });
      this.appRef.tick();
  ...
  onDragDrop(event) {
    ...
    if (
      this.focusedChildComponent &&
      this.focusedChildComponent.id !== environment.DEFAULT_RUNTIME_ID
    ) {
      ...
      this.focusedChildComponent.x =
        event.x - rect.x - this.focusedChildComponent.w / 2;
  ...
  appendNewRuntime(ele) {
    ...
    this.childComponents.splice(this.childComponents.length - 1, 0, {
      ...
      h: ele.h,
      orderNum:
        this.childComponents[this.childComponents.length - 2].orderNum + 1,
      data: {
  ...
  updateFocusedChildComponentPos(focusedChildComponent) {
    const tmp = this.childComponents.filter(
      (i) => i.ref === focusedChildComponent.id
    );
    if (tmp.length <= 0) {
      return;
    }
  ...
  calcDrawLine() {
    ...
    return new Array(this.childComponents.length - 1)
      .fill(this.childComponents.keys())
      .map((val, idx) => {
        ...
        return {
          ...
          x2:
            this.childComponents[idx + 1].x +
            this.childComponents[idx + 1].w / 2,
          y2:
            this.childComponents[idx + 1].y +
            this.childComponents[idx + 1].h / 2,
        };
      });
  }
  ...
  checkIfOutOfPlayground(focusedChildComponent) {
    ...
    if (
      ...
      x + w < 0 ||
      x + w > this.playGroundWidth ||
      y + h < 0 ||
      y + h > this.playGroundHeight
    ) {
  ...
  checkBlockDuplicated(focusedChildComponent) {
    ...
    const duplicatedRect = this.childComponents
      .filter((i) => i.ref !== focusedChildComponent.id)
      .filter((i) => {
        if (
  ...
  onSortChageRequested(event) {
    ...
    if (
      (event.dir === -1 && idx <= 1) ||
      (event.dir === 1 && idx >= this.childComponents.length - 2)
    ) {
      return;
    }

    const orderNumTmp = this.childComponents[idx].orderNum;
    this.childComponents[idx].orderNum =
      this.childComponents[idx + event.dir].orderNum;
  ...
  onBtnSubmitClicked() {
    ...
    const { block, blockLink, runtime } =
      this.splitBlockComponentsFromChildComponents(this.childComponents);
    ...
    Promise.all(promiseList).then((res) => {
      ...
      this._snackBar.open(
        allOk ? environment.MSG_OK : environment.MSG_SOME_FAIL,
        "OK",
        {
          horizontalPosition: "right",
          verticalPosition: "top",
  ...
  splitBlockComponentsFromChildComponents(
    childComponents: RuntimeItemPosModel[]
  ) {
  ...
  onRuntimeDeleteReq(event) {
    if (event.runtime.blockRuntimeId) {
      this.httpService
        .reqDelete(`runtime/${event.runtime.blockRuntimeId}`, null)
        .toPromise()
        .then((res) => {
          this.initData();
        });
