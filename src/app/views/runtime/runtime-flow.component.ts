...
export class RuntimeFlowComponent implements OnInit {
  ...
  onSearchSubmit() {
    this.loadImageList(this.searchImageName).then((res) => {
      // console.log('res', res)
  ...
  onBtnQuickAdd() {
    this.blockLinkedList.push({
      ...
      orderNum:
        this.blockLinkedList.length <= 0
          ? 1
          : Math.max(...this.blockLinkedList.map((i) => i.orderNum)) + 1,
      blockLinkId: 0,
      nextBlockId: "",
      webLinkUrl: "",
      enabled: 1,
  ...
  onRuntimeStateChange(runtime, idx) {
    ...
    if (
      ...
      runtime.blockRuntimeId > 0 &&
      confirm("상태 변경 하시겠습니까?")
    ) {
      ...
      setTimeout(() => {
        ...
        this.runtimeList[idx].containerState =
          this.runtimeList[idx].containerStateOrigin;
        // runtime.containerState = runtime.containerStateOrigin
  ...
  onBtnRemoveRuntime(idx) {
    ...
    if (!this.runtimeList[idx].blockRuntimeId) {
      ...
      this.httpService
        .reqDelete(`runtime/${this.runtimeList[idx].blockRuntimeId}`, null)
        .toPromise()
        .then((res) => {
          this.initData();
        });
          ...
          this.httpService
            .reqPut(
              "/runtimeLink",
              {
                ...
                nextBlockID:
                  blockLink.nextBlockId === "" ? null : blockLink.nextBlockId,
  ...
  onBtnBlockLinkDeleteClicked(targetBlock) {
    ...
    if (targetBlock.registerDatetime) {
      this.httpService
        .reqDelete(`runtimeLink/${targetBlock.blockLinkId}`, null)
        .toPromise()
        .then((res) => {
          this.initData();
        });
